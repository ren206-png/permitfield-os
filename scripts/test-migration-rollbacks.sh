#!/usr/bin/env bash
#
# Phase 1 closeout: tests supabase/migrations_rollback/*.sql (migrations
# 20260806000001-28, the Phase 1 gates 1.0-1.7 set -- see
# docs/PERMITFIELD_OS_EXPANSION_MASTER_PROMPT.md SS7 items 16/18. Migrations
# 29-33 are Gate 2.0 (client portal) and deliberately have no rollback
# files here -- out of Phase 1 scope.
#
# Runs the FULL current migration set forward (supabase db reset --
# whatever's in supabase/migrations/ today, currently 1-33), then walks
# rollback 28 -> 1 in strict reverse order against that live state,
# cumulatively (each rollback runs on top of whatever the previous one
# left behind), stopping immediately on the first error so it's obvious
# which migration's rollback needs fixing. On full success, runs
# `supabase db reset` one more time to confirm the manual rollback walk
# didn't leave the local Postgres container in a state that breaks a
# subsequent clean forward apply.
#
# Usage: bash scripts/test-migration-rollbacks.sh [--stop-at N]
...
for n in $(seq -f "%02g" 28 -1 "$STOP_AT"); do
...
echo "===== all rollbacks from 28 down to ${STOP_AT} applied cleanly. Confirming forward db reset still works ====="
