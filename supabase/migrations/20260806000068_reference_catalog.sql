-- Reference catalog: jurisdictions, authorities, permit types, filings, and
-- verified AcroForm field maps. Moved here verbatim from supabase/seed.sql
-- (formerly its "PART 1: reference data", which that file itself described as
-- "safe in any environment, this is the public-record catalog the product
-- ships"). As seed-only data it never reached production -- seed.sql is not
-- run against hosted projects -- so production had zero jurisdictions and
-- permit types, leaving intake, jurisdiction pages, and PDF fill with nothing
-- to work from. As a migration it ships through the normal deploy path.
--
-- Idempotent so it is a no-op on any database that already has these rows
-- (every local database seeded before this migration existed): the four
-- id-keyed tables use ON CONFLICT (id) DO NOTHING, and permit_form_fields,
-- which has no natural unique key, is inserted only when none of the
-- catalog filings has a field map yet.
--
-- Part 2 of seed.sql (fake users/orgs/applications) stays in seed.sql and
-- remains local-only.
...
-- permit_form_fields has only a random surrogate id, so ON CONFLICT cannot
-- dedupe it; insert the whole verified set only if none of it exists yet.
do $$
begin
  if not exists (
    select 1 from permit_form_fields
    where permit_type_filing_id in (select id from permit_type_filings where id::text like '00000000-0000-0000-0004-%')
  ) then
