...
-- `grant select on <table> to service_role;` statements both closes the
-- gap (service_role could not SELECT before it) and does so without
-- granting anything beyond SELECT (no INSERT/UPDATE/DELETE leaks in).
--
...
-- row; (c) a same-table negative check that service_role still cannot
-- INSERT/UPDATE/DELETE, since this migration is SELECT-only by design
-- (§K.1's migration header: "SELECT only... the bridge layer's five read
-- operations never write to these tables").
--
...
-- Step 6 (regression, negative): this migration is SELECT-only by design
-- -- confirm service_role still cannot INSERT into any of the three
-- tables, i.e. the grant did not accidentally widen beyond what
-- 20260806000032 actually states.
do $$
begin
  insert into organizations (id, name) values ('70000000-0000-0000-0000-00000000000a', 'Should Not Insert');
  raise exception 'FAIL (regression): service_role INSERT succeeded on organizations -- this migration is SELECT-only and must not have granted INSERT';
exception
  when insufficient_privilege then
    raise notice 'PASS (regression): service_role still cannot INSERT into organizations (SELECT-only grant, as intended). (%)', sqlerrm;
end $$;

do $$
begin
  insert into readiness_checklist_items (id, org_id, application_id, title)
  values ('70000000-0000-0000-0000-00000000000b', '20000000-0000-0000-0000-00000000000a',
          '40000000-0000-0000-0000-00000000000a', 'Should Not Insert');
  raise exception 'FAIL (regression): service_role INSERT succeeded on readiness_checklist_items -- this migration is SELECT-only and must not have granted INSERT';
exception
  when insufficient_privilege then
    raise notice 'PASS (regression): service_role still cannot INSERT into readiness_checklist_items (SELECT-only grant, as intended). (%)', sqlerrm;
end $$;

do $$
begin
  -- application_status_history never had a service_role INSERT grant even
  -- before this migration (20260806000022's own header: "No INSERT grant
  -- to authenticated or service_role"), so this is a pre-existing-state
  -- regression check, not a new one this migration is responsible for --
  -- included for symmetry with the other two tables' negative checks.
  insert into application_status_history (org_id, application_id, to_status)
  values ('20000000-0000-0000-0000-00000000000a', '40000000-0000-0000-0000-00000000000a', 'withdrawn');
  raise exception 'FAIL (regression): service_role INSERT succeeded on application_status_history -- this table has never granted service_role INSERT';
exception
  when insufficient_privilege then
    raise notice 'PASS (regression): service_role still cannot INSERT into application_status_history (never granted, unaffected by this migration). (%)', sqlerrm;
end $$;
