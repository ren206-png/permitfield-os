...
| 3 | PDF auto-fill / AcroForm field-filling | **PARTIAL** | `lib/pdf/fill-acroform.ts:32-59` (real `pdf-lib` fill mechanism); `supabase/seed.sql:355-479` — jurisdiction-expansion follow-up has now added verified AcroForm field maps for Surrey (5 fields), Vancouver (6), Richmond (5), Coquitlam (5), Port Coquitlam (5), and Maple Ridge (5) Commercial Tenant Improvement forms, joining Toronto Electrical Service Upgrade's original 3 — **34 verified fields across 7 forms total**; Calgary's and ESA's forms still explicitly have no verified field maps (Calgary flagged in Phase 0 as needing manual page-by-page review; ESA's ICIA-LV form has zero AcroForm fields at all). Caveat first found when Surrey/Vancouver were added, still true today: `lib/inngest/functions/generate-pdf.ts:186-188` downloads the actual template PDF from the `permitfield-form-templates` Storage bucket at runtime, and the bucket is created empty by migration `20260806000017` with no upload step anywhere. **Fixed**: `scripts/seed-storage-templates.ts` (`npm run seed:storage`, see `README.md` Setup) now uploads all 9 real `docs-reference-forms/` PDFs — including Calgary's and ESA's, which have no field-map rows but still need real bytes at their path — to the exact object paths `seed.sql` references. Not yet run against a live Supabase project in this session (no project connected here); still needs a real `SUPABASE_SERVICE_ROLE_KEY` run before this claim is true end-to-end in any given deployment, local or hosted. | Qualified only — "Auto-fills supported AcroForm permit forms (currently Toronto Electrical Service Upgrade, and Commercial Tenant Improvement in Surrey, Vancouver, Richmond, Coquitlam, Port Coquitlam, and Maple Ridge)." **Not** permitted: "automatically fills official permit forms" as a general claim. |
...
| 17 | Deadline / expiry tracking & alerts | **PARTIAL** — contractor-license expiry and permit expiry both built; general deadline/reminder coverage beyond these two target kinds still not built | Deadline/expiry alerts, slice 1: `supabase/migrations/20260806000065_contractor_license_expiry_reminders.sql` adds the `contractor_license_expiring` reminder kind and `contractor` target_kind to the pre-existing `reminder_jobs`/`reminder_delivery_attempts` pipeline (`20260806000057_reminder_jobs.sql`); `app/(app)/contractors/new/actions.ts:62-73` schedules a real reminder_jobs row (30-day lead time via `lib/reminders/contractor-license-schedule.ts`) whenever a contractor is created with a license expiry date; `lib/inngest/functions/reminders.ts:167-` (the hourly poller, same function that already existed) sends it — re-derives eligibility live (`evaluateContractorLicenseReminderEligibility`, `lib/inngest/functions/reminder-eligibility.ts`), broadcasts to every org member via `resolveOrgNotificationRecipients()`, renders `lib/email/templates/contractor-license-expiring.ts`. Slice 2: `supabase/migrations/20260806000066_permit_expiry_reminders.sql` adds a manual `permit_applications.permit_expires_on date` column plus the `permit_expiring` reminder kind and `permit` target_kind (same `reminder_jobs` pipeline); `app/(app)/applications/[id]/permit-expiry-actions.ts`'s `updatePermitExpiryAction` lets any org member set/edit that date from `app/(app)/applications/[id]/permit-expiry-field.tsx`, canceling any stale pending reminder and scheduling a fresh one (30-day lead time via `lib/reminders/permit-expiry-schedule.ts`) whenever a date is present; `lib/inngest/functions/reminders.ts`'s `permit` branch sends it the same way as the contractor branch (`evaluatePermitExpiryReminderEligibility`, `loadPermitSnapshot()`, `lib/email/templates/permit-expiring.ts`). Both slices are gated off by default behind the same flag, `isDeadlineRemindersEnabled()` (`lib/flags.ts:413`, `PERMITFIELD_FF_DEADLINE_REMINDERS=false` in `.env.example`) — must be explicitly enabled per deployment before any email actually sends. Note the permit expiry date is a manually-entered field, not derived from `permit_status_enum`'s dormant `'expired'` value (`lib/permit-status/transitions.ts:41,62,86,112,115-116`, which has zero call sites in `app/`) or from any jurisdiction-modeled validity period. | Qualified only — "Get notified before a contractor's license expires" and "get alerted before your permit expires" are both true once the flag is enabled and (for permits) an expiry date has actually been entered on the application. **Not** permitted: implying alerts exist for any deadline/expiry concept beyond these two target kinds (e.g. no general task/deadline reminders exist). |
