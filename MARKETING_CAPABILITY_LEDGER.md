...
| 17 | Deadline / expiry tracking & alerts | **PARTIAL** — contractor-license expiry only; permit expiry still not built | Deadline/expiry alerts, slice 1: `supabase/migrations/20260806000065_contractor_license_expiry_reminders.sql` adds the `contractor_license_expiring` reminder kind and `contractor` target_kind to the pre-existing `reminder_jobs`/`reminder_delivery_attempts` pipeline (`20260806000057_reminder_jobs.sql`); `app/(app)/contractors/new/actions.ts:62-73` schedules a real reminder_jobs row (30-day lead time via `lib/reminders/contractor-license-schedule.ts`) whenever a contractor is created with a license expiry date; `lib/inngest/functions/reminders.ts:167-` (the hourly poller, same function that already existed) sends it — re-derives eligibility live (`evaluateContractorLicenseReminderEligibility`, `lib/inngest/functions/reminder-eligibility.ts`), broadcasts to every org member via `resolveOrgNotificationRecipients()`, renders `lib/email/templates/contractor-license-expiring.ts`. Gated off by default behind its own flag, `isDeadlineRemindersEnabled()` (`lib/flags.ts:413`, `PERMITFIELD_FF_DEADLINE_REMINDERS=false` in `.env.example`) — must be explicitly enabled per deployment before any email actually sends. Permit expiry itself remains **NOT BUILT**: `'expired'` is still only a manually-set terminal status (`lib/permit-status/transitions.ts:41,62,86,112,115-116`), and `permit_applications` has no expiry-date column or jurisdiction-modeled validity period to compute a reminder from (see the new migration's own header comment for this scope boundary). | Qualified only — "Get notified before a contractor's license expires" is true once the flag is enabled. **Not** permitted: any claim about permit-expiry alerts ("get alerted before your permit expires" is still fabricated) — no such date exists anywhere in the schema. |
...
- "Get notified" / "we'll alert you" copy of any kind, **except** the one narrow, flag-gated case in §17 (contractor-license expiry) — general notifications (§12) remain unbuilt, and permit-expiry alerts remain unbuilt within §17 itself
...
9. Email alerts before a contractor's license expires (§17) — narrow, and only once `PERMITFIELD_FF_DEADLINE_REMINDERS` is actually turned on in a given deployment (defaults to `false`); do not imply permit-expiry alerts exist
